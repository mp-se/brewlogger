...
"""Tests for device endpoints and services."""
...
def test_device_search_by_software_and_mdns(app_client):
    ...
    r = app_client.get(
        "/api/device/?software=Chamber-Controller&mdns=chamber-mdns",
        headers=headers
    )
    assert r.status_code == 200
    data = json.loads(r.text)
...
def test_device_collect_logs_flag(app_client):
    ...
    assert res["collectLogs"] is True
    ...
    assert r.status_code == 201
    res = json.loads(r.text)
    assert res["collectLogs"] is False
