    ...
    for device in devices:
        ...
        for step in device.fermentation_step:
            ...
            if curr_date >= first_date and curr_date <= last_date:
                logger.info(f"Found step that is active; {step.order} => {first_date} - {last_date}, Temp: {step.temp}")
                ...
                if res != None:
                    ...
                    if res["FridgeSet"] != step.temp:
                        logger.info(f"Setting new target temperature to {step.temp}, current {res['FridgeSet']}")
