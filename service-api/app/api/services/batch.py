...
class BatchService(BaseService[models.Batch, schemas.BatchCreate, schemas.BatchUpdate]):
    ...
    def search_chipId(self, chipId: str) -> List[models.Batch]:
        ...
        logger.info("Fetched batches based on chipId=%s, records found %d", chipId, len(objs))
        return objs

    def search_active(self, active: bool) -> List[models.Batch]:
        filters = { "active": active }
        objs: List[self.model] = self.db_session.scalars(select(self.model).filter_by(**filters)).all()
        logger.info("Fetched batches based on active=%s, records found %d", active, len(objs))
        return objs

    def search_chipId_active(self, chipId: str, active: bool) -> List[models.Batch]:
        filters = { "chip_id": chipId, "active": active }
        objs: List[self.model] = self.db_session.scalars(select(self.model).filter_by(**filters)).all()
        logger.info("Fetched batches based on active=%s + chipId=%s, records found %d", active, chipId, len(objs))
        return objs

    def search_brewfatherId(self, id: int) -> List[models.Batch]:
        filters = { "brewfather_id": id }
        objs: List[self.model] = self.db_session.scalars(select(self.model).filter_by(**filters)).all()
        logger.info("Fetched batches based on brewfather_id=%s, records found %d", id, len(objs))
        return objs
